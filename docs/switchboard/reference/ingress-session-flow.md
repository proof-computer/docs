...
description: Visual reference for Switchboard deploy, coordination, and HTTPS traffic paths.
---

# Ingress Session Flow

An ingress session joins local signing, Acurast compute, Hub payment,
Switchboard relays, gateway route state, job-owned TLS, and validator
evidence. These diagrams show the same session from three angles.

## User And CLI Flow

This is the path a developer sees through the CLI. Slipway or another trusted
caller can replace the interactive user shell when it preserves the same
signing, funding, and confirmation boundaries.

```mermaid
sequenceDiagram
  participant Caller as User / CLI / Slipway
...
sequenceDiagram
  ...
  DNS-->>Client: Switchboard gateway address
...
does not terminate application TLS in the default Switchboard path.
