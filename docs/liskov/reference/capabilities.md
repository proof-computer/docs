...
| Retained V5 GitHub source import | v1; repository, ref, workflow, manifest, and optional artifact evidence are exact-bound before draft import |
...
| Retained V5 managed Runtime SSH policy path | Preview on Starter and above; native-image-only `access.ssh.provider.kind: liskov_managed`, with organization operator keys snapshotted into each exact-job attachment |
