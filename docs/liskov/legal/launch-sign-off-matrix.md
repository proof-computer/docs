...
| **BLOCKER** | B2B-only journey | Business-purpose checkbox, organisation/country field, under-18 exclusion, business-oriented marketing; consumer use response | Product/Legal | **In progress — Product/Legal approved `liskov.business-eligibility.v1`, the business-establishment country field, minimal evidence, prospective enforcement and the strict no-consumer-exception response on 4 September 2026. Console, marketing, server enforcement/evidence and public docs are merged and deployed; the live API serves the exact approved contract with `required: true`. Existing external/customer organisations still require re-attestation or documented Product/Legal verification before closure. This proceeds independently of the Stripe canary** |
