...
| **BLOCKER** | Website/e-commerce disclosures | Footer, contact page, checkout and invoices show required identity/address/email/VAT/price information | Legal/Product | **In progress — owner approved GB311456142, hello@proof.computer and price copy on 4 September 2026; site PRs #35–#36 and VAT-safe service/UI/docs changes are merged/deployed. Canary owner action: provide a non-production Liskov runtime configured with Stripe test API/webhook credentials or run an owner-controlled credential-injected local harness; Dashboard access is insufficient** |
| **BLOCKER** | B2B-only journey | Business-purpose checkbox, organisation/country field, under-18 exclusion, business-oriented marketing; consumer use response | Product/Legal | **Complete — Product/Legal approved `liskov.business-eligibility.v1`, the business-establishment country field, minimal evidence, prospective enforcement and the strict no-consumer-exception response on 4 September 2026. Console sign-up/login, server enforcement/evidence and public docs are merged and deployed; the live API serves the exact approved contract with `required: true`. Marketing remains business-oriented, but Product/Legal removed the duplicative legal paragraph from the main Liskov hero on 5 September 2026 (`proof.computer#38`): exact legal copy belongs on sign-up, login and the Terms, not in the hero. Product/Legal verified all seven pre-existing non-personal organisations as internal, leaving no external/customer re-attestation population. No country or acceptance was inferred or backfilled** |
| **BLOCKER** | Clickwrap | Screenshot/test of affirmative acceptance; direct document links; no pre-ticked acceptance | Product/Legal | **In progress — Product/Legal approved the exact split Workspace/Order formation copy on 4 September 2026. Packet m7qa prepares a complete versioned document manifest, exact initially-clear Workspace checkbox, separate later Order acceptance and truthful sign-in notice. Closure still requires the legal suite to be published, every direct link to return 200, the production Terms gate to be enabled and live screenshot/refusal/readback evidence** |
