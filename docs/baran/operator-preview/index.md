...
Acurast processor providers can run a Baran gateway next to their
processors. The gateway gives those processors public HTTPS ingress for
Baran workloads, while application TLS still terminates inside the
Acurast job.

You can earn USDC when paid Baran routes use your gateway and settle.
...
proof baran gateway discover
proof baran gateway status
proof baran gateway upgrade
```
...
Gateway earnings are paid from Baran route sessions after the route is
