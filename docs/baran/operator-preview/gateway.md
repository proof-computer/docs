...
Gateway hosts provide L4/SNI route capacity for Baran. If you already
...
- processors under that manager that can accept Baran workloads
...
Gateways can earn USDC from paid Baran routes. The configured payout
...
```fish
proof baran claimable --recipient '<gateway payout address>'
```

Withdraw only with the payout key and an explicit confirmation:

```fish
proof baran claim --claim-private-key-env OPERATOR_CLAIM_PRIVATE_KEY --yes
```
...
```fish
proof baran gateway setup --admission-file operator-admission.json --yes
```
...
```fish
proof baran gateway discover --manager-id '<manager id>' --public-address '<ip-or-host>'
```

Useful options:

```fish
proof baran gateway discover --limit 20
proof baran gateway discover --smoke-hostname '<sni hostname>'
proof baran gateway discover --write-env .operator-host/operator.env
```
...
```fish
proof baran gateway upgrade --yes
```

Dry-run the Docker Compose commands first:

```fish
proof baran gateway upgrade --dry-run
```
...
does not terminate app TLS in the default Baran path.
