...
A Baran deploy crosses local signing, Acurast compute, Hub funding,
...
`proof baran init` writes directory-local project config:
...
Named contexts live under `~/.baran/` and point at environment variable
names for secrets.

## 2. Preflight

`proof baran preflight --quote` checks the local and network prerequisites
before spend:
...
After the Acurast job starts, it signs the Baran registration material.
...
Baran provisions DNS for the canonical endpoint. The job requests its
